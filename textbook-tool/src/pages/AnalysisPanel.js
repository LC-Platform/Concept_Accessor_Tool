...
export default function AnalysisPanel({
  ...
  const loadConceptMap = async () => {
    ...
    if (conceptCache[key]) { setTaxonomyImg(conceptCache[key]); return; }
    try {
      setIsLoading(true); setTaxonomyImg(null);
      const res = await fetch(`${BASE_URL}/taxonomy-image/${chapterId}/${selectedTerm.domain_id}`);
      const img = res.ok ? `${BASE_URL}/taxonomy-image/${chapterId}/${selectedTerm.domain_id}` : null;
      setTaxonomyImg(img);
      setConceptCache(prev => ({ ...prev, [key]: img }));
  ...
  return (
    <div style={{ padding: panelPad, overflowY: "auto", height: "100%" }}>
      ...
      {activeTab === "ConceptMap" && (
        <div>
          {isLoading ? (
            <div style={{ textAlign: "center", padding: "30px 0", color: "#888" }}>
              <div className="spinner" style={{ margin: "0 auto 8px" }} />
              <div style={{ fontSize: 13 }}>Loading concept map…</div>
            </div>
          ) : taxonomyImg ? (
            <div>
              <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                <button onClick={() => setZoom(z => z + 0.2)} style={{ padding: "6px 12px", borderRadius: M.rS, border: `1px solid ${M.border}`, background: "#fff", cursor: "pointer" }}>＋</button>
                <button onClick={() => setZoom(z => Math.max(1, z - 0.2))} style={{ padding: "6px 12px", borderRadius: M.rS, border: `1px solid ${M.border}`, background: "#fff", cursor: "pointer" }}>－</button>
                <button onClick={() => setZoom(1)} style={{ padding: "6px 12px", borderRadius: M.rS, border: `1px solid ${M.border}`, background: "#fff", cursor: "pointer", fontSize: 12 }}>Reset</button>
                <button onClick={() => setConceptFullscreen(true)} style={{ marginLeft: "auto", padding: "6px 14px", borderRadius: M.rS, border: `1px solid ${M.border}`, background: "#fff", cursor: "pointer", fontSize: 12 }}>⛶ Full</button>
              </div>
              <img src={taxonomyImg} alt="Concept map" style={{ transform: `scale(${zoom})`, maxWidth: "100%", borderRadius: M.rS, transformOrigin: "top left" }} onDoubleClick={() => setZoom(z => z === 1 ? 1.8 : 1)} />
            </div>
          ) : (
            <div style={{ textAlign: "center", padding: "40px 0", color: "#aaa" }}>
              <div style={{ fontSize: 32, marginBottom: 10 }}>🗺️</div>
              <div style={{ fontSize: 14 }}>No concept map available</div>
            </div>
          )}
        </div>
      )}
      ...
      {/* Concept map fullscreen */}
      {conceptFullscreen && taxonomyImg && (
        <div onClick={() => setConceptFullscreen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.92)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <button onClick={() => setConceptFullscreen(false)} style={{ position: "absolute", top: 20, right: 20, width: 36, height: 36, borderRadius: "50%", border: "none", background: "#fff", cursor: "pointer", fontSize: 16 }}>✕</button>
          <img src={taxonomyImg} alt="Concept Map" onClick={e => e.stopPropagation()} style={{ maxWidth: "92%", maxHeight: "88%", objectFit: "contain", borderRadius: M.rS }} />
        </div>
      )}
